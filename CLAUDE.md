...
Migrations live in `migrations/` as numbered SQL files (`001_*.sql` through `NNN_*.sql`, currently 50 files — recent: 045 `tm_course_api_cache`, 046 `tm_gameplans` (**applied to prod 2026-07-16**), 047 `tm_user_courses` (**applied to prod 2026-07-16**), 048 `tm_native_push_tokens` (NOT applied — needs APNs send path), 049 `tm_ota_bundles` (self-hosted OTA backend, see 07-16 log). Earlier: 028 added `tm_osm_cache` (durable Supabase cache of OSM/Overpass golf geometry, public Overpass hit at most once per course), 029 added `tm_elevation_cache` (USGS 3DEP DEM for plays-like), 030 added `tm_users.gender`, 031 added `tm_outings` per-player tee ratings, 032 added `tm_handicap_history` (365-day Low-HI), 033 added `tm_rounds.hole_handicaps` (solo rounds handicap like outing rounds), 034 added `tm_practice_logs` (data→practice loop), 035 added `tm_outings(status, host_id)` indexes (apply CONCURRENTLY, on its own), 036 added `tm_outing_participants.score_version` (F.5 optimistic-concurrency version column — now USED by the S2 on-behalf write), 037 added `tm_idempotency_keys` (F.5 S3 — offline-replay dedup; **applied to prod 2026-06-29**), 038 added guest support to `tm_outing_participants` (F.5 S4 — `user_id` nullable + `is_guest`/`guest_id`/`guest_name` + shape CHECK + backfill; **applied to prod 2026-06-29, backfilled 13 existing guests**), 039 added `tm_rounds.putts`/`first_putts` + `tm_users.sg_baseline` (SG v2 putt facts + baseline toggle; **applied to prod 2026-07-02** by Dale), 040 added `tm_users.shot_shape`/`typical_miss`/`distance_miss` (AI-caddie tendencies; **applied to prod 2026-07-02** by Dale), 041 added `tm_outing_participants.putts`/`first_putts` (live putt capture in outings, self-score only; **applied to prod 2026-07-06**), 042 added `tm_outing_participants.shots` jsonb (live outing SHOT capture → read-time Strokes Gained OTT/APP/ARG; self-score-only write on `PUT /:code/scores`, synced to `tm_rounds.shots` at `/end`; **applied to prod 2026-07-07**; pipeline e2e-verified — capture UX is the next build, see [[wiki/synthesis/eagle-eye-walk-and-confirm-spec-2026-07-07]]), 043 added `tm_course_holes` (curated per-course tee/green/aim overrides — the "Map this course" editor's store; editor UI SHIPPED 2026-07-10, see [[wiki/synthesis/map-this-course-editor-build-spec-2026-07-10]]), 044 added `tm_rounds.course_id` (Phase 3 flyover shot editor loads hole geometry by course id; **applied to prod 2026-07-10** via Supabase MCP, 9 outing rounds backfilled — see [[wiki/synthesis/phase3-premium-differentiators-build-spec-2026-07-10]])). **SG v2 MERGED to main 2026-07-03 (PR #1)** — SG engine (read-time, never persisted), putt chips + ShotSheet + post-hoc putt editor, Stats SG card, SG-aware Practice signals, AI Caddie chat (`ANTHROPIC_API_KEY` + optional `CADDIE_MODEL` in Vercel prod; per-user rate limit; 10-round putting honesty gate). **F.5 IS COMPLETE — S2–S7 are all LIVE on the beta** (2026-06-29): `SCORING_OCC_ONBEHALF=1`, `SCORING_IDEMPOTENCY=1`, `SCORING_GUEST_ROWS=1`, `SCORING_AGG_READ_FROM_ROWS=1`, `SCORING_DESIGNATED=1`, and `SCORING_STATE_WRITES_OFF=1` are set in Vercel prod env alongside `SCORING_READ_FROM_ROWS=1`. `db.tx` + `SELECT FOR UPDATE` confirmed against the real Supabase pooler. **S4 design**: guest rows use `user_id = NULL` so every existing `user_id`-keyed exclusion (recent matches, rounds, h2h, handicap) keeps excluding them with no query change. **S5 (no migration)**: friends-live, season, leagues/standings, and CSV derive each player's total/scores from the authoritative rows instead of `state.total` (flag `SCORING_AGG_READ_FROM_ROWS`); the participant list + withdrawn/no_show flags + format config stay state-sourced (that's what survives into S7). Verified a no-op on real data — 45/45 participants across 38 closed outings had `row_total == state_total`. **S6 (no migration)**: designated-scorer mode — `PUT /:code/scoring-mode` sets `state.scoring_mode 'open'|'designated'`; when `SCORING_DESIGNATED=1` AND mode is designated, `/scores/host` drops the same-group bypass so only host + assigned marker enter OTHERS' scores (players always self-score their own; scorer-vs-self conflict uses the S2 chip). Per-outing opt-in + global flag = double safety; flag off or mode 'open' ⇒ today's behavior. Client adds the scorer banner + non-scorer "who's scoring" indicator + host assign-nudge; hand-off reuses the Edit Groups marker UI. **S7 (no migration) — the cutover, LIVE 2026-06-29 (`SCORING_STATE_WRITES_OFF=1`):** score writes stop syncing into `state.participants` (state keeps config only — withdrawn/no_show/markers/groups/teams); the row is the SOLE score store. Completed the reader coverage so this is safe: `/public` + `/:code` derive GUEST scores from the guest row (by guest_id), and `/end`'s podium/highlights derive total+scores from rows for app-users AND guests (fixed split-brain trap #6 — ceremony had sorted off stale state). Dead `/scores/marker` returns 410. Verified through the Express app + live on prod (scores→rows, state stays 0, `/public` correct from rows incl. guest). **Reversible** (remove the env var → dual-write resumes; no reader used state scores so the gap is invisible). The whole "never lose your round" data-model rework is now done. Browser UI verified (2026-06-29). **Only residual: a real on-course round on the native iOS shell (POST-LAUNCH-TODO #25) — now a post-hoc confidence check, not a gate, since state was a redundant copy not a true fallback (a 200 means the row committed; the offline queue + idempotency protect writes).** Whole-F.5 reversibility: each `SCORING_*` flag is an independent off-ramp. **Repair (2026-06-28): `004_tm_games.sql`** was added to fix a broken rebuild path — `tm_games` was referenced by 005/023 but never created by any migration (it only existed on live Supabase), so a from-scratch replay failed at 005. The repair sorts after `004_avatar.sql` and before `005`; a clean replay of all migrations now succeeds (verified 37/37 on a fresh Postgres). Apply in order on a fresh database:
