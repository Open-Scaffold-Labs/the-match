...
**F.5 progress:** S4 (guests → rows) and S5 (flip readers to row-derived) both **SHIPPED + LIVE 2026-06-29**. S5: `friends-live`, `season`, `leagues/standings`, CSV now derive totals/scores from rows (`SCORING_AGG_READ_FROM_ROWS=1`); verified a no-op on real data (45/45 participants, 38 closed outings, `row_total == state_total`). **Next: S6** (conflict-UX polish + optional designated-scorer mode — the inline chip already ships from S2; S6 is the optional "one group scorer" default for league/tournament play, plus any multi-writer UX refinement) → **S7** (irreversible cutover: stop writing `state` scores, neutralize the dead `/scores/marker`, default the read flags on + retire them). **S7 is the only irreversible step and is gated on the S2/S3 real-round device test below.** S7 stays last.
