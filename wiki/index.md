...
- [[synthesis/next-session-handoff-2026-07-09]] — **ACTIVE handoff** — EE HUD single-row re-layout + LiveOuting header→one MatchMenu dropdown + grass on Match tab; match-end correctness (idempotent `/end`, rivalries from completed scorecards only, `{save}` flag + save-or-discard prompt) + active-match guard discards via `/cancel`; prod seed cleanup of Matt's account (handicap 8.5→14.2, rivalries rebuilt); next build = Start-Match Phase 0 (nav restructure) · created 2026-07-09
