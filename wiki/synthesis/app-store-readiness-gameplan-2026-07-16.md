...
- ✅ **Code-split shipped.** App.jsx now lazy-loads the heavy non-initial routes (EagleEye, Outing, MyBag, Profile, PGAScores, OnboardingWizard, PublicLeaderboard, PrintResults) behind a `<Suspense fallback={<Splash/>}>`; Home + Login stay eager. **Startup bundle: 1,175 KB → 588 KB (~50% smaller).** EagleEye (98 KB), Outing (367 KB), etc. are now on-demand chunks. Verified on-device: tapping Play → EagleEye chunk loads + renders; tapping Match → Outing chunk loads + renders; navigation (incl. Eagle Eye's custom back) intact.
- ✅ **API round-trip PROVEN end-to-end.** Logged into the real account on the sim: Home rendered live backend data (profile, 14.2 HCP, following/followers, W-L record, clubs) and the Match tab fetched matches. Confirms CapacitorHttp bypasses the single-origin CORS and authenticated calls reach `the-match-roan.vercel.app`.
