...
### Updated priority list (after runtime findings)

Re-prioritized with R1-R8:

1. **R3** (tee duplicates) — visible to every user picking a course on a real course. ~10 lines of dedupe logic. **Top priority.**
2. **R4** (Home/Stats handicap inconsistency) — confusing, easy to fix once decided. **High.**
3. **R5** (background contrast) — affects readability on multiple screens. Add overlays to text-heavy panels. **High.**
4. **R2** (modal escapes container) — makes course-pick feel broken on any non-phone view. **Medium-high.**
5. **R7** (Tour "RD 0" label) — looks like a bug to anyone seeing it pre-tournament. **Medium.**
6. **R1** (Match-tab overlap) — only visible on >430px viewports. Real for desktop preview, lower for phone-only users. **Medium.**
7. **R6** (tee-time duplicates indistinguishable) — UX clarity. **Medium.**
8. **R8** (GolfNow stub copy) — wording change to commit-or-remove the "coming soon." **Low.**
9. Then back to original B1 (CLAUDE.md feature status), B7 (console.log), B5 (rate limit), U1 (page splits), B8 (React Router), F2 (Big Team Battle), F8 (per-hole scoring).
