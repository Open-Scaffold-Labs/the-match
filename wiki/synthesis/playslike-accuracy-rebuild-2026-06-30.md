...
**Still to build (Option B — aim-point trigger):** when a player drops an aim point short of the pin, plays-like should compute to THAT point (a real shot) instead of the pin. The aim currently lives inside HoleMapGL and isn't surfaced to EagleEye — needs an `onAimChange` callback + EagleEye state + a UX decision (does the hero number follow the aim, or only the plays-like chip). Default (no user aim) stays Option A (to the pin, capped). Honest residual: the ideal is to scale wind by club/trajectory (high wedge > low driver); carry-cap is the pragmatic minimum-correct version.
