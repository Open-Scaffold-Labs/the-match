...
**1. No logout / sign-out anywhere in the app.**
...
**2. No `/settings` route — silent SPA fallback to home.**
...
**3. Course name truncation: "Pebble Beach Gl" everywhere.**
...
**5. No "Forgot PIN" / PIN reset flow.**
...
1. **Privacy + logout + delete-account** (POST-LAUNCH-TODO #11 + this audit's bug #1). All three are blockers for App Store submission, can be one Settings-page session.
2. **Forgot-PIN + Settings page polish.** Adjacent to #1, completes the auth UX.
3. **Course name truncation root-cause.** Quick win that improves the perceived polish across the whole app.

After those three, the question becomes engagement loops vs Eagle Eye depth — which deserves its own product-strategy session with brainstorming.
