...
2. **Desktop leagues/commissioner layout.** The global app frame is capped at `maxWidth: 430` (`App.jsx`). Leagues have commissioners who run them from desktops on the course (Matt, this arc), so league/commissioner routes should relax the frame + use responsive inner grids. Functional today but narrow on desktop. *Scoped, not started.* (A first responsiveness pass was done earlier — task #36 — but the frame cap remains.)
