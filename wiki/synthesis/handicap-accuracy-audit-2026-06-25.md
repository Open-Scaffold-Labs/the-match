...
- ⚠️→✅ **9-hole rounds — was CORRUPTING the index; now guarded (`roundDifferential`, 2026-06-26).** Matt asked the right question: *what happens to a handicap after a 9-hole round?* Tracing it exposed a real bug, not just a missing feature. 9-hole rounds ARE creatable (ActiveRound slices to 9; CreateWizard sets `coursePar: holes===9 ? 36 : 72`) and passed `isRoundCompleted` (≥9 scores). The differential then compared a ~9-hole gross (~40) against either the 18-hole Course Rating (~70 → a hugely **negative** differential that **crashes** the Index toward a false plus-handicap) or the 9-hole par (~36 → a too-low differential that **drags it down**). Either way a single 9-hole round corrupted the Index downward. **Fix:** `roundDifferential` now returns null for any sub-18 round (detected by `scores.length` < 18, or `course_par` < 55 when scores absent), so 9-hole rounds can't enter the 18-hole Index. Proven by `ninehole-solo-si.test.cjs` (a "great" 9-hole round with an 18-hole rating leaves the Index unchanged; three 9-hole rounds alone yield no Index). **Still a follow-up to COUNT them WHS-correctly** (the major apps do): needs 9-hole tee Course/Slope ratings captured → the expected-9 method. Excluding is the safe do-no-harm until then — better no count than a corrupt one.
