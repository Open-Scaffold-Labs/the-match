...
**What happens**: The Limitless Stack preflight grew a comprehensive set of mechanical checks — files committed, repos pushed, timestamps consistent, tools byte-equal with canonical, NotebookLM dedupe sweep, sync state file fresh. By 2026-05-07 it had ~12 such checks. None of them looked at the *content* of the documents being checked. The result: this session discovered that the-match's CLAUDE.md "Feature status" table had been claiming push notifications were "🔲 Next" for the entire week after they shipped, that `wiki/index.md` was missing 7 pages, that `wiki/overview.md` had been a `YYYY-MM-DD` template placeholder for 10 days, and that `HIGH-PRIORITY-TODO.md` (a security-critical JWT_SECRET rotation) had sat 3 days past its stated deadline. Every preflight in those 10 days reported READY/WARN with no mention of any of it.
