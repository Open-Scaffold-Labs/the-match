...
- **OTA (Capgo) — BLOCKED, backed out.** `@capgo/capacitor-updater` installed + wired (config + `notifyAppReady`), but its heavy native SwiftPM tree (Alamofire/BigInt/ZIPFoundation/CryptoSwift) **hung `xcodebuild` package resolution twice** in this CLI env (cleared cache + retried — still hung). Removed it to keep the native build green; the JS wiring was reverted. OTA capability still owed — revisit by resolving Capgo's packages in the Xcode GUI (often succeeds where CLI hangs) or picking another OTA tool. NOTE: OTA must be in the FIRST submitted binary (can't be added via OTA later).
