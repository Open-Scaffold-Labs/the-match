...
- **Runtime findings (fix before submit):** (1) 🔴 ~11.5s first paint + splash auto-hides at 600ms → ~10s blank dark screen. Root cause: 24MB ONNX wasm + MapLibre on the launch path. Fix = splash `launchAutoHide:false` + hide after first paint, AND code-split ONNX/Eagle-Eye off launch. (2) 🟡 API round-trip not yet proven (login renders, but no authenticated call verified — no test creds).
