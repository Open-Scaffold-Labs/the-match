...
- `marketing/README.md` documents the one-time deploy: second Vercel project off this repo with Root Directory=`marketing` (static), add the domain, CNAME `the-match` → `cname.vercel-dns.com` on openscaffoldlabs.com DNS, then swap social bios/GolfNow materials to this URL. **Deploy + DNS = Matt/Dale action, not done in this session.**
