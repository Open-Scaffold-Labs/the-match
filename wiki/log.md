...
- **Does NOT unblock Matt's current build** — no OTA bundle is live, so this reaches TestFlight only via a NEW native build. **DALE: cut the next build from main (has this fix); it'll work without needing VITE_API_ORIGIN in your shell.** Matt's immediate workaround: sign in on the web/PWA (the-match-roan.vercel.app), backend + account are fine.
