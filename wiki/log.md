...
- **Competitive deep research** (5 parallel agents; report: ~/Projects/The-Match-Competitive-Threat-Assessment.docx). Verdict: "best brain in the field, playing off the wrong tee." Confirmed: NOBODY has in-app conversational voice (18Birdies/Arccos partnered with Meta AI glasses instead — $300+ hw); mid-market night-before plans are open (Arccos AI Strategy is hw-gated; DECADE manual; Upgame×TrackMan elite-only). Ranked gaps: (1) no native app/Watch — wearables 30–38% of golf GPS and the review-deciding factor; (2) no official GHIN — USGA GPA program is attainable paperwork (Barkie, 15 ratings, got approved; TheGrint issues GHIN numbers); (3) course data — mapping (not GPS) drives accuracy, iGolf 40k+/GolfLogix greens vs our OSM+free API; (4) scale; (5) auto shot detection (Arccos Air is sensorless now). Barkie ships scorecard OCR ("ScoreShot") — validates the queued ingester.
- **Apple Watch strategy + implementation plan** (~/Projects/The-Match-AppleWatch-Strategy.docx). Dale's thesis: the watch is a microphone with a haptic engine, not a small screen — speech in; haptic/glance/AirPods out (quiet speaker = design instruction). Standalone-first (watch → /api/v1 directly, phone optional — replaces the Walking-Mode pocket shield). Voice tier reuses /api/voice/parse verbatim: zero new endpoints. Phases W0–W4 (~9–12 wks): shell+TestFlight → glance/crown scoring → wrist voice → geofenced haptic tee-briefs (GamePlan glances = voice Phase-2 hook) → App Intents/Hey Siri + store. Gate: Apple dev account + Friday.
- **Swing Intelligence strategy** (~/Projects/The-Match-SwingIntelligence-Strategy.docx). Research: swing-video market commoditizing; universal flaw = fault-grading with no outcome data (Mustard scored a Tiger swing 9.3); NOBODY joins swing video to SG (18Birdies has both halves, unjoined); vision-LLMs hallucinate faults — architecture must be pose-model + LLM narrator (SAMI pattern); Sportsbox acquired by DeChambeau group, Gemini "SAMI" coach ships late 2026 (the clock). Strategy: build the swing→ball→score JOIN (video body metrics + launch-monitor module + SG), anchored to the round loop; **Dale's multi-year range video archive = founding dataset**; bulk archive import ("Swing Timeline") is an unclaimed market wedge. Phases V0–V3; V0 = pilot on Dale's archive, zero production risk.
- New tasks: #21 launch-monitor import module (Garmin Premium API exists as later path), #22–23 done (papers), #20 scorecard ingester queued. Next session first moves: driveway/Friday test results → merge self-report → W0 (Apple dev account) + USGA GPA application + Golf Intelligence trial.
...
- **Native geolocation shim** (`client/src/lib/geolocation.js`): native → `@capacitor/geolocation` (explicit high-accuracy Core Location); **web path delegates 1:1 to `navigator.geolocation` so web/PWA is byte-identical** (zero beta risk). Adopted in the two LIVE rangefinder watches — EagleEye (`startGpsWatch`/`requestLocation`/resurrection re-fix/`clearWatch`) + ActiveRound — via contract-preserving swaps; the tuned watch-resurrection/watchdog logic is untouched. Honest note: foreground GPS in WKWebView already uses native Core Location, so the real remaining win is a **background-geolocation plugin** (locked-screen tracking) + on-device tuning — deferred to device testing.
- **Native push (APNs)**: `@capacitor/push-notifications` + `registerNativePush()` in `lib/push.js` (guarded native; posts APNs token to server after sign-in); `isNativeShell()` now recognizes Capacitor (suppresses web-push "add to home screen" nudges in the native app). Server: `POST/DELETE /api/notifications/register-native` + **migration 048 `tm_native_push_tokens`** (NOT applied to prod yet). iOS: AppDelegate APNs-token forwarding hooks + Info.plist `remote-notification` background mode + `App.entitlements` (aps-environment) template. **Remaining (needs Apple account):** enable the Push capability in Xcode (ties to `com.openscaffoldlabs.thematch` App ID) + APNs .p8 key + the server-side APNs SEND path (token store is ready; sender not built — needs the key).
- **OTA (Capgo) — BLOCKED, backed out. ROOT CAUSE (audited 2026-07-16, evidence-based):** SwiftPM package resolution stalls indefinitely cloning the **Alamofire** repo, which `@capgo/capacitor-updater` requires as a transitive dep (its HTTP client for downloading update bundles — declared `.upToNextMajor(from: "5.12.0")` in Capgo's Package.swift). Verified: `git ls-remote` on all Capgo deps returns <1s (NOT unreachable); Capgo's version constraints are compatible with the app's capacitor-swift-pm 8.4.2 (NOT a version conflict); tiny dep Version (332K) and even ZIPFoundation (35M) clone fine — but a FULL clone of Alamofire trickled ~8M/120s (~68 KB/s) and never completed, while a shallow clone hit ~625 KB/s. So the transfer rate to Alamofire is severely VARIABLE/THROTTLED, and SwiftPM needs the full clone → "Fetching from Alamofire" blocks the whole build. It is NOT the encryption feature, NOT a Capgo bug, NOT a version/reachability problem. **Correcting the earlier note here:** it is NOT a general "heavy tree" (ZIPFoundation is heavy and cloned fine) and Capgo does NOT depend on CryptoSwift (that was an unverified guess) — the crypto dep is BigInt; the blocker is the Alamofire clone specifically. **Not definitively determined:** whether the throttle is GitHub rate-limiting this IP after many repeated clone attempts today (self-inflicted, transient) or general network congestion — likely transient. **Fix paths:** retry (rate is variable — may succeed), resolve in the Xcode GUI on Matt's machine (fresh network state), or pre-seed the SwiftPM cache once Alamofire clones. Removed to keep the native build green; JS wiring reverted. OTA still owed and MUST be in the FIRST submitted binary (can't be added via OTA later).
- **Verified**: client `lint` (exit 0) + `build` green; `node --check` on notifications.js; `cap sync`; **native `xcodebuild` → BUILD SUCCEEDED** (geolocation + push, Capgo removed); app **installs + runs** on the iPhone 17 Pro sim (login renders, no crash from the new native code). Bundle id `com.openscaffoldlabs.thematch`.
...
- **Capacitor 8.4.2** added to the `client` workspace; `npx cap add ios` generated `client/ios` (Capacitor 8 = Swift Package Manager, no CocoaPods). `client/capacitor.config.json` bundles the web assets (NOT a remote-URL wrapper — avoids the Guideline 4.2 lazy-wrapper rejection) — appId `com.openscaffoldlabs.thematch` (set to match FireHazmat's namespace).
...
- **Ran on iPhone 17 Pro simulator** — login screen renders natively (screenshot). Prod bundle `VITE_API_ORIGIN=https://the-match-roan.vercel.app` (backend `/health` live). **CapacitorHttp enabled** to bypass the server's single-origin CORS (`cors({origin: CLIENT_ORIGIN})` rejects `capacitor://localhost`).
- **Runtime findings (CORRECTED after measuring — earlier alarm was wrong):** the "~11.5s first paint" was a **cold-booted-simulator** artifact, NOT the app. Warm relaunch = login rendered by **~2s**. Heavy deps are ALREADY lazy — MapLibre (`await import('maplibre-gl')`) + 24MB ONNX/background-removal (`await import('@imgly/background-removal')`) are dynamic imports, not on the startup path. So there is no "24MB on launch" problem; that diagnosis was a mistake (raised off one cold-start number without measuring). **Splash-until-ready shipped anyway** (`launchAutoHide:false` + `hideSplash()` from App.jsx first paint + gold spinner) — correct native hygiene against any blank flash, not a band-aid. **Code-split SHIPPED:** App.jsx lazy-loads heavy non-initial routes (EagleEye/Outing/MyBag/Profile/PGAScores/OnboardingWizard/PublicLeaderboard/PrintResults) behind `<Suspense fallback={<Splash/>}>`; Home+Login eager. Startup bundle **1,175KB → 588KB (~50%)**. Verified on device: Play→EagleEye chunk + Match→Outing chunk load & render; nav intact. **API round-trip PROVEN:** logged into real account on sim → Home shows live backend data (profile, 14.2 HCP, W-L, clubs), Match tab fetches matches → CapacitorHttp bypasses CORS, authenticated calls reach the-match-roan.vercel.app end-to-end.
- **Bundle ID set** to `com.openscaffoldlabs.thematch` (2026-07-16) to match FireHazmat's `com.openscaffoldlabs.firehazmat` namespace — updated in capacitor.config.json + both Xcode configs, verified in the built `.app`. **Merged to main** (52c205d) combining with Dale's 07-15 work (only wiki/log.md conflicted; both entries kept). **Still NOT done**: native GPS/cam/mic ride the webview bridge (unverified on device); native APNs push not implemented.
...
## [2026-07-07 PM8] feat | ANALYZE camera rangefinder CUT (`dfc52eb`) + SG-vs-Dale-memo reconciliation
- Assessed the parked ANALYZE (Matt: un-park?) → NO-GO, cut entirely. It was an AI-vision guess, not a rangefinder (the server told Claude NOT to estimate distance from the image when GPS yardage is known); monocular distance isn't credible at golf range (research: scale ambiguity, ~20m demonstrated ceiling vs 137-183m; iPhone LiDAR ~5-10m); its slope read is LESS accurate than our shipped USGS-DEM plays-like; it duplicated the data-driven Caddie; + camera-permission friction + App-Store 2.3 accuracy-claim risk. Removed CameraModal+ResultSheet+last-analysis card+state (client) and /analyze+buildPlayerProfile+estimateAlt+unused Anthropic/sgPromptBlock imports (server); sgPromptBlock still powers the Caddie. 9+/501-; lint/build/tests/node--check all green
- Dale's "Performance Intelligence / Strokes Gained" memo reconciled vs shipped: the SG v2 engine (server/src/lib/sg) ALREADY implements the memo's Phases 1-3 at engine level — 4-category OTT/APP/ARG per-shot chains + putting + Total/T2G, handicap-band baselines, worst-APP-bucket biggest-leak, trend series; the Caddie already injects the SG block w/ a 10-round putting-reliability gate. The memo appears to predate SG v2 — worth telling Dale
- LIVE-DB verify (read-only; 62 rounds / 10 users): 0 rounds with shot-chain data, 1 with putts; outings 84 participants / 2 with putts. NOT a bug — 60 of 62 rounds PREDATE putt-capture (migration 039, 2026-07-02) and outings DO flow into tm_rounds (outing_id + INSERT). So putting SG fills on its own as rounds get played; the one genuinely-unbuilt gap = per-shot lie+distance capture (no UI; with_shots=0) → the real Phase-2 lever for OTT/APP/ARG. Caddie prompt-deepening (trends + leak coaching) deferred as premature without data

## [2026-07-07 PM9] fix | CORRECTION to PM8 — per-shot capture IS built (solo); the real gap is outings
- Matt pushed back (twice — first putting, then shots); verified in code, not inferred: per-shot capture is SHIPPED. ActiveRound's `ShotSheet` captures club + lie (tee/fairway/rough/sand/trouble) + distance-to-pin, saved to `tm_rounds.shots` → the SG OTT/APP/ARG engine. PM8's "no UI / genuinely-unbuilt" claim was WRONG — the same empty-DB-column→"unbuilt" inference error made on putting
- ACCURATE gap: shot capture is SOLO-ROUND ONLY. Outings capture putts but NOT shots — the outing→tm_rounds INSERT (outings.js ~2295) omits the `shots` column, and no Outing/ component renders ShotSheet/onAddShot/toPin. Outings are the majority of play (43 outings vs 62 rounds), so OTT/APP/ARG stay empty in practice; also the feature is ~4 days old + opt-in per shot. So the 4-category engine is fully built end-to-end for SOLO; the real lever = wire the existing ShotSheet into the outing scorecard + add `shots` to the outing→round sync (+ adoption), NOT a from-scratch build
- LESSON (self): in this codebase an empty DB column means new/unused, NOT unbuilt. Read the capture code before claiming a feature is missing. Two wrong "not built" calls this session traced to trusting a count over reading the flow

## [2026-07-07 PM10] feat | Outing shot capture SHIPPED (`366547c`) — per-shot {lie,toPin} → SG, self-only
- The PM9 gap closed at BUILD level: the existing ShotSheet, extracted to a shared components/scorecard/ShotSheet.jsx, now renders in the LiveOuting self-score modal (self-only, opt-in, per-hole, add-only), so per-shot {lie,toPin} capture reaches the SG engine in OUTINGS, not just solo. migration 042 adds tm_outing_participants.shots (APPLIED to prod, verified jsonb); PUT /:code/scores accepts a self-only `shots` key (touched only when the body carries it — mirrors putts; never wipes, never gates scoring); the outing→tm_rounds sync carries shots so the read-time engine walks complete chains → OTT/APP/ARG. Research-grounded: per-shot SG is inherently self-only/per-hole-confirmed; no app group-captures shots; manual logging must be opt-in + never block play
- VERIFIED: server vitest 95 pass (12 new shot-facts tests incl cleaned-shots→OTT/APP/ARG + incomplete-chain→null); client lint/build/tests green; node --check ok; migration column confirmed; deploy 366547c /health ok + served bundle grep 'Shot Log'/'Log Shot'. Extraction boundary bug (CLUBS `]` was line 43, I sliced 26-42) caught + fixed before push
- NOT VERIFIED (audit-before-claim, honest): the live HTTP round-trip (PUT w/ shots → row written), the real outing /end sync → tm_rounds.shots, and the BROWSER UX (log a shot in a real outing, save, confirm persist + SG lights up). Build+unit+migration+deploy verified; end-to-end is a browser/e2e walkthrough STILL TO RUN. Follow-up: prefill/edit existing shots on modal re-open (v1 is add-only)

## [2026-07-07 PM11] feat | Outing shot capture e2e-VERIFIED + decision: build EE walk-and-confirm (spec written); premium ShotSheet abandoned
- E2E-verified the PM10 outing shot pipeline LIVE (browser + in-page API + DB + real engine): PUT /:code/scores shots → tm_outing_participants.shots; POST /:code/end → tm_rounds.shots; shipped roundSG on a 9-complete-chain synced round → sgOTT 2.34 / sgAPP 17.1 / shotHolesCounted 9 (magnitudes synthetic; categories PROVEN to populate). The 4 throwaway shot-e2e test outings DB-cleaned afterward (host=1, name like 'shot-e2e%'); Matt's outing rounds back to 19
- Researched best-in-class shot ENTRY (2 agents): the leaders NEVER type/step a distance — GPS-derived, confirm-not-build; dark "instrument" visual for the measurement act. the-match's Eagle Eye (live GPS-to-pin) is the unfair advantage no phone-only rival has without sensors. Built a premium manual ShotSheet (dark/stepper, no keyboard) then REVERTED it per Matt ("if the leaders abandoned it so should we") — plain ShotSheet.jsx stays as the manual fallback only
- DECISION (Matt): build **real-time Eagle Eye "walk-and-confirm" shot capture** as the PRIMARY path — the "biggest name in golf apps" move. Full build spec + Plan-agent architecture map (file:line, slices, risks) in [[synthesis/eagle-eye-walk-and-confirm-spec-2026-07-07]]. Slice 0 (shared buffer) + Slice 1 (outing) first; reuses the shipped shots pipeline; NO server change for S1. Honest v1 limit: lie auto-detect = tee/fairway default + override (no fairway/bunker polygons yet). NEXT SESSION picks this up

## [2026-07-08 AM1] feat | Eagle Eye "walk-and-confirm" shot capture BUILT — Slices 0/1/3 + solo clean-on-save; live-verified
- Built the real-time EE capture decided in PM11. **Slice 0** (`3fcbe28`): shared per-hole shot buffer `client/src/lib/shot-capture.js` + solo façade in `solo-round.js` + `ScoreModal` rewired to hydrate from the buffer (byte-identical manual path) + 20 unit tests. **Slice 1** (`bb83047`): `ShotCaptureSheet` (dark instrument sheet) + LOG SHOT HUD pill + **plays-like club rec** (advise on `computePlaysLike(gpsToGreen)`, store RAW toPin for SG) + "forgot-to-log" backfill button + trust nudges (>500y / non-decreasing distance). **Opened to ALL EE rounds** (`78d46b7`): EE self-discovers a solo round via `readSavedSoloRound`; solo writes ride the ONE round blob + fire `tm-solo-shots` → ActiveRound re-hydrates (kills the clobber race). **Slice 3** (`7da5c3b`): ray-cast `pointInPolygon` in geo.js + on-green guard + solo clean-on-save (`cleanShotsForRound` on POST /rounds).
- Process: 3 research agents (usability/accuracy/visual bar-to-beat, cited) + a Plan agent → a bulletproof, audited plan + risk register in [[synthesis/eagle-eye-walk-and-confirm-progress-2026-07-07]]. Matt's steer mid-build: the bar is BEATING the leaders, not parity with our old code (→ plays-like club rec, using the engine we already own); and capture must work for EVERY EE round, not just outings.
- VERIFIED: client lint+build green; tests geo 38 / clubModel 16 / shot-capture 20; server 97 (shot-facts 14); node --check ok. **LIVE on the beta (Claude-in-Chrome):** LOG SHOT shows for a solo round; a confirmed shot wrote {lie:"tee",toPin:165,club:"7i"} into the round blob (correct SG shape); map renders; plays-like computes. Test shot cleaned up. Honest correction logged: I called EE "loads clean" off the console while the map was mid-tile-load — overclaim; it does render.
- PENDING (Matt's on-course pass, real GPS — desktop Chrome geolocation DENIED): the GPS→plays-like HERO + the on-green warning firing (both code+unit verified). Remaining feature: Slice 4 (stretch) — fairway/bunker Overpass polygons for full auto-lie. Handoff [[synthesis/next-session-handoff-2026-07-08]] (+ rollup regenerated). Carry-forward invariants: lie key `recovery` (never `trouble`); toPin=raw gpsToGreen; currentHole 1-idx vs 0-idx; SG gate shots+putts===score; solo one-blob + `tm-solo-shots` sync.

## [2026-07-09] feat+fix | EE HUD row + MatchMenu dropdown + grass Match tab + match-end correctness + prod seed cleanup + Start-Match plan
- **Eagle Eye HUD** re-laid-out into a single bottom control row: DIAL|BIG toggle (left, shrunk to clear overlap), distance rangefinder card centered, LOG SHOT (right) as a dark-glass full-pill matching the toggle/nav pills (was a full-width solid-gold button); hole selector nudged down slightly. Commits `5724a51`/`225a1f9`/`dac1165`/`66b1a13`/`04fb5a6`/`71b0482`.
- **LiveOuting header** rebuilt: the scattered rainbow emoji pills (📡 Share / $ Side Bets / 💬 Chat / ⚙ Manage / End Match + settings chips) → ONE premium **MatchMenu** dropdown (grouped Settings/People/Play/Manage, red danger End Match, chat-unread dot on the collapsed trigger). MENU sits LEFT, gold **GET DISTANCES** pill RIGHT, on one row. Design-critique skill run first; no-emoji line-icon set. `2919d60`→`c04d243`→`1ea7ca9` (interim GET-DISTANCES placements `ae0e8cc`/`0949f08` reverted).
- **Grass background** extended to the Match/Scorecard surface: `App.jsx` grassTab + TabPanel `opaque={false}` (`125b37a`), OutingHub root → transparent (`51d45d1`).
- **Correctness — active-match guard** (`2fd3934`): "already in a match" guard now DISCARDS the old match via `POST /:code/cancel` (not the heavy `/end`) and refuses to proceed if the discard fails ⇒ a 2nd live match can never be created. Root cause of Matt's two-live-matches: `/end` swallowed errors + proceeded.
- **Correctness — match end** (`f94e6ff`, server+client): `/end` is **idempotent** (only active→closed writes stats; re-ending tallies nothing — fixes "ended 4× ⇒ rivalry 4×"); rivalries built from **COMPLETED scorecards only**; `{save}` flag ⇒ `save:false` closes without recording; client `endMatch()` gates on `isRoundFullyScored()` and, when unfinished, shows a **Save & record / End without saving / Keep playing** sheet. Implements Matt's rule: h2h only from actually-scored rounds.
- **Prod DB cleanup** (Matt user_id 1, Supabase MCP `bqjdiixkygslaryxcyfg`): walkthrough throwaway outing 87 cancelled (two-live → one); 13 bulk-seeded championship rounds deleted → handicap recomputed via the app's own `handicap.js` from 6 real rounds → **8.5 → 14.2**; 13 "Seed:" outings (32-44) + their match_history + 5 seed h2h deleted; match_history for incomplete/0-round outings 68+76 removed and Matt's h2h rebuilt from completed matches only → Daniel 1-1, James 1-0 (Tiger/Mary/Pat/Sean were unfinished-match artifacts; Tiger=real friend kept where real). Outings 68+76 left as-is per Matt.
- **Start-Match plan**: 3 research agents (competitor start-funnel + on-course UX) + Explore code map → [[synthesis/start-match-unified-flow-plan-2026-07-09]]. Agreed target nav **Home · Match · ▶Play · Profile · Tour** (Eagle Eye→Play; merge Leagues into Match w/ Matches|Leagues toggle; Profile own tab). Next build = **Phase 0 (nav restructure)**. Handoff [[synthesis/next-session-handoff-2026-07-09]] (rollup regenerated).
- Honest: this session skipped the mandatory session-start roll-call/preflight.

## [2026-07-10] feat | Phase 0 nav restructure SHIPPED (`c194432`) — Home · Match · ▶Play · Profile · Tour
- Session opened with roll-call per CLAUDE.md: WARN (1 uncommitted vault file = pre-existing dormant HoleMapGL "Map this course" editor draft, LEFT uncommitted; main NotebookLM bucket at 49/50 sources).
- **Leagues → Match tab** (Phase 0a): LEAGUES bottom tab removed; `Outing.jsx` hub now renders OutingHub OR `Leagues` behind a `Matches | Leagues` segmented pill (new `MatchSegToggle`, threaded into both hub headers via a new `headerAccessory` prop on OutingHub + Leagues/LeaguesHub). Segment persisted (`tm-match-seg`); any bottom-nav tap resets it to Matches (same 2026-05-07 convention Home used — and the escape hatch from the full-screen Leagues paywall, which has no back control). Stale persisted `tm-last-tab='leagues'` remaps to OUTING. League "+ New event" still opens CreateWizard via App's pendingLeagueId (App no longer needs the tab switch). Leagues' desktop-only 1180px breakout retired with the tab (App-Store target is the 430px frame).
- **Profile → own tab** (Phase 0b): new `pages/Profile.jsx` owns its data fetching (/api/profile, stats/summary, rounds?limit=20, follows/counts, my-rivalries, games) and renders the now-EXPORTED `ProfileView` + `EditProfileModal` from Home.jsx plus the Practice/Caddie/PlayerCard/FriendProfile overlays — screen pixel-identical to the old sub-view. Mounted ONLY while active (plain conditional, NOT lazy-keep-alive: ProfileView portals a fixed overlay to <body>, which escapes TabPanel's display:none). Home dropped the whole `homeView`/'tm-home-view'/tabPressedAt-reset machinery + its rounds/rivalries fetches (2 fewer API calls on Home load); its two profile entry points now `onNavigate('profile')`. ProfileView back arrow → Home.
- **Labels** (Phase 0c): OUTING 'Scorecard' → 'Match'; EYE 'Eagle Eye' → 'Play' (Eagle Eye stays as the in-screen brand); new `IconUser` for Profile.
- VERIFIED: client build + lint green (the full gate; no server files changed). NOT yet verified on-device: the toggle/tab flow on Matt's phone — next beta pass. Per [[synthesis/start-match-unified-flow-plan-2026-07-09]] Phase 0; next build = Phase 1 (Play start funnel, unify the two CoursePickers, wire solo into sharedCourse).

## [2026-07-10 PM] feat | Play funnel Phase 1 BUILT — unified CoursePicker + solo→sharedCourse + PlayStart start screen
- Per [[synthesis/play-funnel-phase1-build-spec-2026-07-10]] (written this session from 2 fresh agents: market start-funnel micro-UX research w/ citations + Plan-agent seam map at HEAD). Matt greenlit Phase 1 with the "bulletproof, beat the leaders" mandate.
- **S1 (`f15bcb8`)**: `components/CoursePicker.jsx` — ONE picker, two VERBATIM JSX variants (EE dark sheet / wizard-solo light inline) over a shared `useCourseSearch` data layer (both now send lat/lng → server distance sort; sheet keeps its client-side miles re-sort). Emission contracts unchanged. CreateWizard re-exports for compat; EagleEye's in-file picker deleted.
- **S2 (`f15bcb8`)**: solo SetupSheet passes `onCourseTeeSelected` + `gender` (threaded App→Outing→ActiveRound) — solo picks now seed sharedCourse (EE auto-loads the course) and dedupe tees gender-correctly (was silently male). `lib/eye-hole.js` extracted; hole memory reset to 1 on pick. courseId/courseTee survive in the solo blob.
- **S3**: `lib/course-recents.js` (tm-recent-courses, upsert cap 10, lastTee memory; written by both picker variants + LiveOuting seed) · `pages/PlayStart.jsx` replaces the EE Welcome hero — visible nearest-recent(≤5mi)/last-played confirm card (never invisible auto-start), other-recents chips, 9|18, Solo|Match (solo default), gold START, Resume card (startSoloRound refuses double-start), "Rangefinder only" zero-scoring path, Enable Location kept · `startSoloRound()` in lib/solo-round.js writes the exact ActiveRound blob shape + fires `tm-solo-started` · EE `startRound()` — solo: blob+recents+hole-1+course seed; match: ensureSingleActive guard → light POST /api/outings (server contract verified: only `name` required, outings.js:365-420; 9-hole via par-sum≤40 rule) → one-time MATCH-code invite chip on the map → App `pendingOpenCode` mounts Match tab hidden → Outing opens live view WITHOUT join POST (host already participant).
- **Research distilled into the build**: 2-tap target when the default is right; zero interstitials policy; visible confirm (wrong-course lock-on = market's #1 trust killer); per-course tee memory (no leader does it well). Decisions flagged to Matt in spec §3: recents-based nearest (no /nearby API exists — fast-follow logged), invite chip (not auto CodeShare), GPS-only toggle stays Phase 3.
- VERIFIED (audit-before-claim run): client build + lint green on every push; npm test 11/11 (NOTE: the larger node --test suites weren't run); server route contract read-verified; no server files changed. **NOT VERIFIED — honest**: zero browser/device walkthroughs this session (funnel flow, picker pixel-fidelity, match e2e, activeScoring publish, league wizard regression). Full device checklist = spec §7.

## [2026-07-10 PM2] fix | PlayStart reachable over an active course view (`d8ff9b5`)
- Matt's first device pass hit the gap immediately: persisted sharedCourse ⇒ courseCtx always set ⇒ map always rendered ⇒ PlayStart unreachable (EE back = tab exit). Fix: `showStart` state in EagleEye forces the start screen over the map; entered by tapping the COURSE NAME in the header (that tap previously opened the picker directly — picker still one tap away via "Not here?"); PlayStart grows a "Back to map" pill when a course view sits behind; cleared on back/any pick/any start/sharedCourse sync. Header stays a solid band (not the floating map gradient) while the start screen shows. Build+lint green; on-device re-check is Matt's next step.

## [2026-07-10 PM3] fix+feat | Play session model corrected from Matt's device pass (`d8ff9b5`→`f941261`)
- Matt's on-device feedback drove 4 fast iterations, all build+lint verified and pushed:
  1. `d8ff9b5` PlayStart reachable over an active course view (course-name tap in header; "Back to map" pill).
  2. `e4a746a` Start screen de-cluttered — hole selector, GPS/wind/temp pills, SCORECARD, RINGS/ARCS/BAG were gated on courseCtx only and floated over PlayStart; all map-HUD chrome now also requires !showStart.
  3. `1ab230f` Copy: overline → "EAGLE EYE · AI-POWERED CADDIE" (Matt: it's a caddie, not a rangefinder); the false "Rangefinder only — just yardages, no scoring" link → "Just browse a course map — no round started". **FRAMING RULE (Matt): Eagle Eye is ONE surface — yardages + scoring together (Scorecard wired in); there is no separate 'rangefinder mode'. Never write copy implying one.** Recorded in PlayStart's header comment.
  4. `f941261` **Session model flipped**: the map is only the default when a round is ACTIVE (solo blob or live-outing activeScoring); otherwise every Play-tab entry opens the START screen (new `isActive` prop re-arms showStart on entry — EE stays mounted, so mount-time state went stale). sharedCourse hydration no longer forces the map; GET DISTANCES/picks/starts still land on the map. Back button: browsing → exit + re-arm; active round → bottom sheet (End round on the scorecard → the PROPER save flows / Exit keep round / Keep playing) — never a silent end.
- Known tradeoff (documented): a live match with the Outing tab sitting at 'hub' publishes no activeScoring, so Play would open on the start screen; the active-match guard still prevents double-starts. Revisit with the Phase-2 single active-round session object.

## [2026-07-10 PM4] feat | Instant NEARBY COURSES in the picker (`0431fd5`) — closes the "blank sheet" gap
- Matt: picker opened as an empty black sheet until a whole word was typed — "extremely cheap", below the leaders (they list the closest courses on open). GolfCourseAPI is text-search only (the known no-/nearby gap from the Phase-1 spec §3), so nearby comes from OSM: new allowlisted `nearby` query type on the existing hardened Overpass proxy (`leisure=golf_course` node/way/relation → names + centers, `out center`), riding tm_osm_cache + mirror fallbacks; client snaps the bbox to a 0.1° grid (±0.25° cell) so a whole area shares one cached row — Overpass hit at most once per cell.
- Both CoursePicker variants (EE sheet + wizard/solo inline) now open with a NEARBY COURSES list (miles, sorted, ≤30 mi, ≤12 rows) before any typing; typing swaps to live search. Tap → `resolveNearbyCourse`: name search (then simplified name), accept the result nearest the OSM PIN (not the user) within 5 mi; else the name drops into the search field as a visible search — a tap never dead-ends. Sheet autoFocus removed (keyboard covered the list). Honest loading/none/no-location states.
- Gate: node --check (server file touched) + client build + lint green. NOT verified live: an actual Overpass nearby fetch + resolve on device — Matt's next pass. The Phase-1 spec §3 "nearby fast-follow" is now DONE (OSM-backed, not vendor-backed).

## [2026-07-10 PM5] fix | Device-pass polish round 2 (`8c6e3d2`, `bbf24d6`)
- `8c6e3d2` Invite chip moved INTO the header stack under the hole selector (was absolutely positioned at a fixed offset and covered the hole pill — Matt: "lazy work"; lesson: overlay chrome goes in the layout flow, not pinned offsets).
- `bbf24d6` **End round directly from the map**: the EE back-prompt's "End round" now dispatches `tm-request-end-round` → LiveOuting runs its REAL `endMatch()` (identical guards: fully-scored confirm, unfinished save-or-discard sheet — now PORTALED to <body> so it renders through the tab switch; idempotent `/end` + `{save}`), ActiveRound jumps solo to summary/save; EE switches to the Match tab same tap. One end flow, two entry points — zero duplicated end logic. Was: back → scorecard → find End Match in menu → sheet (3 steps).

## [2026-07-10 PM6] feat | Phase 2 BUILT — active-round session + QuickScoreSheet + GPS nudge (`1dcd5b8`, `fc14f8a`, `e0c0af0`)
- Same methodology as Phase 1 (Matt: "same mindset... execute"): 2 fresh agents (on-course scoring/auto-advance micro-UX research w/ citations — key finding: NOBODY ships toast+undo for hole advance, and GPS auto-advance is the market's complaint magnet; Plan agent seam map at HEAD with every session writer/clear site at file:line) → spec [[synthesis/play-oncourse-phase2-build-spec-2026-07-10]] → slices → audit.
- **P2-A/B/C (`1dcd5b8`)**: `lib/active-round-session.js` — single "is a round active?" index (doctrine: solo truth=blob, match truth=server, session=index, NEVER load-bearing; merge-upsert writes, code-guarded clears, readers self-heal). Wired at 9 start/join sites + 5 end/clear sites; remote ends reconciled via LiveOuting status polls + throttled /recent check on Play entry. Play's predicates + capture scope + PlayStart resume cards read it — **the hub blind spot is closed**. App boot re-mounts round owners from the session; tm-solo-started mounts the Match tab.
- **P2-D/E (`fc14f8a`)**: `components/scorecard/QuickScoreSheet.jsx` — score the hole from the Play map. OWNER-RENDERED portal (LiveOuting/ActiveRound render it, EE toggles via App quickSheet state) ⇒ zero forked scoring: match saves run the REAL saveScore (offline queue, idempotency, OCC chip, putt/shot rides, celebrations — all portals, all render over the map); solo saves run setScore+autosave blob. EE "SCORE HOLE N" pill; sheet follows EE's hole ONE-WAY (no drive-back races); 1-idx→0-idx converted once at the owner boundary. v1 scope: self-score only; "Full scorecard →" jumps to Match tab; no save→advance.
- **P2-F (`e0c0af0`)**: consent-based GPS hole-advance nudge — trusted fix + ≤45yd to next tee + closer than current green + 3 ticks → dismissible chip (dismissal per course+hole); never automatic/silent/last-hole. Deviation logged: no undo toast (chip IS consent; toast+undo only matters for auto-advance).
- Gate: client build + lint green on every push; client-only (no server changes in all of Phase 2). **NOT VERIFIED (honest)**: every runtime behavior — session matrix, sheet e2e (server write, offline replay, OCC, celebrations), nudge GPS behavior. Full device matrix = spec §7.
- **KNOWN BLOCKER for wiki→NotebookLM**: main bucket hit 50/50 at PM4's refresh; THIS session's new spec page cannot be added until sources are consolidated (handoffs-rollup pattern / IGNORED list) — top handoff item for next session.

## [2026-07-10 PM7] fix | TDZ crash class caught on device → fixed + gate hardened (`8d6b205`)
- Matt hit "cannot access 'vt' before initialization" (white error popup) — the P2-F nudge effect's DEP ARRAY read `showStart`, declared later in the component body; dep arrays evaluate synchronously at render ⇒ TDZ ReferenceError that a clean build AND the existing lint rules both miss (same class as the documented 2026-05-02 loadOuting TDZ and the 2026-06-06 "build passing is NOT enough" lesson).
- Fixed by ordering (showStart/roundActive up next to currentHole; nudge block below). **Gate hardened per the self-improvement rule: `no-use-before-define` (variables-only) is now the THIRD error rule in client/eslint.config.js** (after no-undef and jsx-no-undef, each added after its own shipped crash). The new rule immediately caught a SECOND live crash I'd shipped in `0431fd5`: the inline CoursePicker called `useNearbyCourses(coords)` above the `coords` declaration — the wizard/solo picker would have white-screened on open. Pre-existing lexical-order noise (fetchWeather, SLOT_LABELS_TOGGLE, FollowList/Leagues style consts) moved above first use — zero behavior change.
- Lesson (append to the running theme): every gate rule here was purchased with a shipped crash; dep arrays and hook ARGUMENTS are render-time code — a const's position in the component body matters.

## [2026-07-10 PM8] feat+fix | Save=advance on QuickScoreSheet (`04bb504`) + geometry trust gate (`ad6eb83`)
- **Save=advance** (Matt: sheet stayed open after Save): a successful sheet save now closes + advances the map to the next hole via the existing eyeHoleNudge plumbing (one-way flow preserved). Match: awaits saveScore — failed/OCC-cancelled saves keep the sheet open; queued-offline advances. Solo: advances the solo scorecard's active hole in step. Never advances past the last hole.
- **Geometry trust gate** (Matt's Bayonne catch: hero FROM TEE 365 = scorecard vs drawn line 229 = mis-matched OSM point): per hole, if drawn tee→green distance disagrees with the CARD yardage by >20% AND >30yds, the hole's tee/green/line are dropped at read time (gated useMemo over renamed *Raw position states) → map degrades to the existing unmapped-hole path + a quiet "didn't pass verification" note. Curated tm_course_holes overrides exempt (authoritative). Doctrine: the scorecard is the truth anchor; measured geometry must agree before display. Two TDZ near-misses during this build were caught BY the new no-use-before-define gate before commit (the rule is already paying rent).
- **NEXT (Matt chose "Both")**: finish the **Map-this-course editor** — the permanent per-course fix. The uncommitted HoleMapGL draft (~41 lines: editMode/editDraft/editCandidates/onMapTap props + guarded edit layers + tap handler) is preserved on Matt's Mac working tree; still needed: the page-side flow in EagleEye (entry point in the header/course area → per-hole tee→green step-through with OSM candidate snapping → PUT /api/courses/:id/holes → reload overrides). Server GET/PUT + migration 043 already exist. First build next session.

## [2026-07-10 PM9] revert | Geometry trust gate REVERTED (`4831b2b` reverts `ad6eb83`) — major regression on device
- Matt's device: Bayonne stopped picking up holes entirely + app slowed. Root cause of the regression: the gate filtered the SHARED holePositions/greenPositions maps that ALL consumers read — including the OSM gap-fill/fetch logic that decides which holes still need geometry. Gated holes look permanently missing to that logic → refetch loop (the slowdown) while the gate keeps hiding results (no holes). Diagnostic gold from the failure: Matt saw the "didn't pass verification" note on the map ⇒ MANY Bayonne holes fail card-vs-drawn verification, i.e. Bayonne's OSM tee/green matching is systematically wrong (likely off-by-N hole assignment), not a single bad point.
- LESSON (framing for the redesign): a display-trust gate must live at the RENDER boundary (what HoleMapGL draws / what the hero quotes), never in the data layer that drives fetching. Revert restores pre-gate behavior (hole 1 shows the wrong 229 line again — accepted vs no map).
- NEXT SESSION (unchanged but sharpened): build the Map-this-course editor (permanent fix; HoleMapGL draft preserved uncommitted on Matt's Mac) and, if a gate is still wanted, re-do it render-side only with the Bayonne systematic-mismatch case as the test.

## [2026-07-10 AM] refactor | NotebookLM main bucket consolidated 50/50 → 38 + rollup (unblocks new wiki pages)
- Blocker #1 from [[synthesis/next-session-handoff-2026-07-10]] cleared before the editor build.
- Deleted 7 ORPHAN sources with no corresponding the-match wiki file: `top-contender-gameplan-2026-06-01` + `ui-excellence-playbook-2026-06-01` (repo files removed in `328e5d3`, the undone 06-01 session), and `centralize-the-user-shape`, `the-match-is-NOT-dark-theme`, `opensalon`, `mlav1114`, `team-tasks` (Hub-vault files, never in this repo — covered by the Hub's own notebooks).
- Rolled the five COMPLETE F.5 sub-specs into ONE source: new [[synthesis/f5-specs-rollup]] (handoffs-rollup pattern); manifest `exclude_paths` += `wiki/synthesis/f5-never-lose-your-round-build-spec-` and `wiki/synthesis/f5-s`; the 5 individual sources deleted.
- index.md: added the rollup; removed the dead `HIGH-PRIORITY-TODO` link (file deleted in `b50f4a2`, index still pointed at it).
- Refresh re-run so [[synthesis/play-oncourse-phase2-build-spec-2026-07-10]] (yesterday's upload_failed) finally syncs. Target end-state: 40/50, 10 free slots.

## [2026-07-10 AM2] feat | "Map this course" editor SHIPPED (`ad429fe`→`41333fa`) — per-hole tee/green overrides, built on the uncommitted draft
- The permanent fix for systematically-wrong OSM matching (Bayonne), replacing the reverted trust gate. Spec + LIVE checklist: [[synthesis/map-this-course-editor-build-spec-2026-07-10]] (Plan-agent seam map at `c2cfe29`; Matt's decisions: per-hole PUT, SUBTLE entry, no aim v1, 15yd snap).
- **S0** `ad429fe`: committed the 41-line HoleMapGL draft (props + guarded edit layers + tap handler). **S1** `0e22c6a`: `redrawEdit` body (candidate dots kind-colored, draft pins gold/green, dashed line; all sources emptied when !editMode — normal mode pixel-identical). **S2** `cac3cf2`: edit session state (geometry block — TDZ-safe), SUBTLE entry (auto "Fine-tune this course?" chip ONLY when a hole lacks confident geometry or card-vs-drawn mismatch >max(25yd,12%) — computed at render, never written back; discreet pencil by the course name always), per-hole tap-tee→tap-green step-through with 15yd snap to the editor's OWN teegreen fetch, editor HUD replaces the hole pill. **S3** `33287cd`: green tap = save & advance — per-hole `PUT /api/courses/:id/holes` + the SAME local additive override merge as the GET effect (map exact immediately, no refetch); failed PUT keeps the hole open with a retry chip; exit prompts discard only for a half-placed dirty hole. **S4** `3d90537`+`41333fa`: nudge/chips/QuickScoreSheet suppressed mid-edit, aim marker inert (incl. mid-edit recreation), camera top-down north-up while editing (re-framed on the flip itself).
- Anti-refetch-loop doctrine held: the editor only ever ADDS keys to holePositions/greenPositions/holeGeometries via the shipped merge; wrongness detection is render-time only.
- Gate green per slice (build + lint incl. no-use-before-define). **ZERO runtime verification yet** — spec §6 device checklist (Bayonne mapping pass, accurate-course no-chip, kill+reload persistence, failed-PUT retry, normal-mode pixel parity) is the done-gate. Server untouched (migration 043 + routes already live).

## [2026-07-10 AM3] feat | Phase 3 premium differentiators SHIPPED (`d9da51e`→`6ba1f30`) — flyover shot editor + plays-like trust + upgrade seam
- Parent plan [[synthesis/start-match-unified-flow-plan-2026-07-09]] §5 Phase 3; spec + LIVE checklist [[synthesis/phase3-premium-differentiators-build-spec-2026-07-10]]. Three agents: post-round-editor competitive research (the flyover-IS-the-editor thesis — unclaimed in market; sign-off/SG-ready patterns; complaint traps), GPS-only + plays-like research, Plan-agent seam map at `b7a5891`.
- **Corrected a wiki premise**: "Rangefinder only" does NOT exist in PlayStart (stale comment only; doctrine = EE is ONE surface). GPS-only therefore shipped as the **browse→scoring upgrade seam** (`d46e625` ⚑): starting a round on the course you're actively browsing keeps your hole; fresh course still opens hole 1. ⚑ Grazes the Phase-1 hole-1 doctrine — Matt's device-pass judgment call.
- **Server**: `PATCH /api/rounds/:id/shots` (owner-only, re-cleaned, atomic putts ride-along) + `{lat,lon}` pin passthrough in cleanHoleShots (SG-indifference pinned by test) (`d9da51e`); **migration 044 `tm_rounds.course_id` — APPLIED TO PROD via Supabase MCP, 9/50 rounds backfilled from outings** + both writers (`ca0d9d0`); `round_ids` map added to the /end ceremony summary (`c49e795`). 107/107 vitest incl. 8 new route-level PATCH tests (real requireAuth w/ signed JWT; db singleton patched through node's require registry — vi.mock can't reach the CJS chain) (`d5198a3`).
- **Client**: HoleMapGL parallel SHOT seam (`1e15e0f`, redrawEdit contract, EagleEye untouched); **ShotEditor page** (`acb0d2c`) — per-hole satellite pins + ball path, tap-to-add/select/move/delete, lie chips, shared PuttChips, SG-ready chain indicator + round progress, PATCH-on-advance w/ retry chip, overrides-first geometry (course-editor synergy), zero-capture + no-map fallbacks; entry points (`c49e795`): solo post-save optional review offer, EndMatchScreen "Review your shots", RoundScorecard sibling of Add-putts; flyover camera = the existing per-hole cinematic. Plays-like: verified best-practice already shipped; added per-factor source captions (`5b3b7a6`).
- **Audits**: audit-before-claim (honest split in spec §4 — zero client runtime execution yet; §7 device checklist is the done-gate) + design-critique (44px targets, aria-labels, contrast fix `6ba1f30`; follow-ups: jump-to-hole, post-hoc club-first friction, pin drag, green view for putts, shareable flyover clip).

## [2026-07-10 PM] fix+feat | End-of-match ceremony redesigned (`6565a0d`) — Matt's screenshot showed it washed out over the grass photo
...
## [2026-07-10 PM2] fix | Course search outage — root-caused, leak killed, resilience shipped (`2547673`, `272dbf1`)
- Matt: "GPS off doesn't let you search a course." REPRODUCED live in Chrome: search returned NOTHING for any query. Root cause chain: (1) the course-data vendor was returning HTTP 429 rate-limit; (2) the server's `d.courses || []` swallowed it into 200 {courses:[]} — silent-empty, search looked dead app-wide; (3) with GPS ON the OSM nearby list masks it, so it presented as a GPS bug; (4) **the actual quota burner: the Phase-1 sheet picker had live `gps` in the search effect's deps — a vendor search PER GPS TICK (~1/sec) while the sheet was open with a query typed** (on-course device testing = hundreds of calls).
- Fixes: **leak killed** (`272dbf1` — coords frozen at sheet open; live fix still drives distance labels via gpsRef); **migration 045 APPLIED to prod** — `tm_course_search_cache` (7d TTL) + `tm_course_cache` (30d TTL), cache-through + STALE-IF-ERROR on /search and /:id (tm_osm_cache doctrine; vendor hit ≤ once per query/course per TTL, stale rows serve through outages); vendor errors now honest 502 + message (502 not 503 — client auto-retries 503s and would hammer the limited vendor); both pickers render search errors + a no-matches state instead of silent blank. 6 new route tests (113/113).
- VERIFIED live end-to-end after the vendor limit reset: picker search renders results with GPS off; cache filling. Map "not rendering" in session screenshots = WebGL canvas capture artifact (canvas live, NAIP tiles 200 in ~100ms) — not an app bug.

## [2026-07-11] fix | On-course GPS "slow / not tracking" — watch resurrection + per-tick redraw cut (`693645f`)
- Matt's round report (07-10): whole app janky, yardages stale, GPS pill repeatedly dropping OUT of gold; screen locked between shots; mostly yardage use. Explore-agent diff (07-08→HEAD) + symptom triage converged on TWO causes:
- **(1) The fix itself was dying: iOS suspends WKWebView on screen lock and the geolocation watch silently stops delivering on resume.** The wake-lock visibilitychange handler re-acquired the SCREEN lock but nothing re-armed the WATCH — every pocket-and-wake cycle left a stale/dead fix (pill drops out of gold, yardage stuck). Fix: resurrection layer — visibility/pageshow resume bounces the watch AND takes a one-shot re-fix (`maximumAge: 3000`) so the first number after unlock is instant; a 10s-interval watchdog bounces any watch with a >20s fix gap while visible (also self-heals Low Power Mode starvation); restarts rate-limited to 1/15s. No-ops for users who never enabled GPS.
- **(2) Accidental per-tick full aim-overlay redraw:** `bagArcsData = bagArcsOn ? … : []` minted a new [] every render; every GPS tick re-renders EagleEye (now ~3300 lines after the build week) → HoleMapGL's reference-keyed bagArcs effect fired per tick → full redrawAim (multiple setData + label DOM) once a second all round with arcs OFF. Fix: module-level `EMPTY_BAG_ARCS` + the gps-anchored overlays (landing zone/arcs/rings) now refresh through the puck effect ONLY when one is visible — intended follow-the-player behavior preserved, default walking state skips it.
- Agent also confirmed NOT-regressions (don't re-chase): watch lifecycle + syncPuck/halo byte-identical since 07-08; elevation fetch still 11m-keyed; edit/shot seams inert in normal mode. Logged follow-up (not built): isolate the gps tick from the full 3300-line re-render (structural, post-device-validation).
- Gate green. Runtime validation = Matt's next round: pill should re-gold within ~1-3s of unlocking, yardage instant. Honest residual: iOS may still take 1-2s to re-fix after long pocket stints — physics of GPS re-acquisition, but no longer minutes of dead watch.

## [2026-07-16] fix | Scoreless "-71 rounds" bug — solo save guard + reader hardening + prod cleanup (`35bf692` on main)
- Matt: two most recent matches showed "-71", no scores ever entered, polluting 3-RND AVG + Avg Score. Root cause: `POST /api/rounds` had ZERO validation — the outing `/end` path guards recorded rounds (9+ holes, every hole > 0, total > 0) but the solo POST never did. Trigger: Eagle Eye's "End round" back-prompt (shipped 07-10) jumps a solo round to the summary phase, whose Save button fired with all 18 holes at 0. Two all-zero rows (ids 163 Pebble Creek, 164 Beacon Hill, both 2026-07-10, user 1, par 71 → total 0 rendered as -71).
- Verified before fixing: rounds 163/164 were the ONLY rows in the entire prod DB with any zero hole or total ≤ 0; handicap was never affected (`isRoundCompleted` already excluded them).
- Fix: (1) server — POST /api/rounds 400s when no hole has a score > 0 (blocks stale cached clients); (2) client — ScorecardSummary Save disabled w/ "No scores entered" at 0 scored holes + saveRound() backstop; (3) reader hardening `total > 0` on rounds GET, stats /summary (avgScore/bestScore — bestScore would have shown 0), profile avg3, friends profile rounds; (4) prod cleanup — both rows deleted via Supabase MCP, verified 0 bad rows left, Matt's numbers restored (avg3 84.7, avg 87.7, best 82).
- Process note: first commit accidentally landed on the `feat/ios-native-capabilities` spike branch — cherry-picked to main (beta rule: main IS the test surface), branch reset to Matt's exact prior state (b95b102 + 4 uncommitted SwiftPM files intact).
- Open product question from Matt (answered in-session, not yet built): partial rounds — score some holes, average against holes PLAYED not all 18. Currently a partial save counts the raw sum against full-18 stats. Filed to POST-LAUNCH consideration.

## [2026-07-16 PM] query+schema | Partial-rounds build spec written + session cleanup
- Matt greenlit planning partial-round support ("built right from the start, think of what could go wrong"). Full spec: [[synthesis/partial-rounds-stats-build-spec-2026-07-16]] — **NEXT BUILD**. Core: one shared `server/src/lib/roundMath.js` (playedCount/parPlayed/equiv18/isFullRound), averages become 18-hole-equivalent par-aware over ≥9-played qualifying rounds, "47 thru 10 (+5)" display, Best Round = real full totals only, outing `/end` extends to record ≥9-played partials (adds hole_pars to that INSERT), handicap UNTOUCHED v1 (Tier-2 `HANDICAP_PARTIAL_POSTING` flag deferred). 14-item failure-mode register; prod parity run is the ship gate (full-round numbers must not move).
- Consumer audit findings baked into the spec: achievements already gate on filled===18 (safe); handicap already 9-hole-posts via WHS expected-score and excludes 10–17 (correct-by-exclusion); friends activity feed computes `total − course_par` in SQL with no guard (needs the roundMath treatment); outing `/end` silently drops 15-of-18 rounds today.
- Cleanup: 3 unindexed synthesis pages added to wiki/index.md (app-store-readiness-gameplan-2026-07-16, voice-interface-build-spec-2026-07-15, gameday-strategy-build-spec-2026-07-15); `tools/notebooklm-wiki-refresh.py` synced FROM LimitlessStack canonical (canonical newer: short-id JSON-parse fix + OFH handoff prunes; py_compile verified).

## [2026-07-16 PM2] feat | Partial rounds (holes-played-aware stats) BUILT + SHIPPED — spec §6 slices S0–S5 all landed
- Built in a linked git worktree (`.wt-partial-rounds`, removed after push) so Matt's in-flight `feat/ios-native-capabilities` checkout was never touched.
- **S0** `server/src/lib/roundMath.js` — THE shared math (playedCount/isFullRound/isQualifying/parPlayed/toParThrough/par18/equiv18); equiv18 returns the raw total EXACTLY for full 18s (parity by construction, not float luck). 21 unit tests incl. pg-string coercion, 9-hole-course doubling, pro-rate fallback, null shields.
- **S1** readers on the lib: stats `/summary` (avg = mean equiv18 over ≥9-played; best = real totals over FULL rounds only + `bestScoreHoles` for the "(9)" label), profile `avg3` (last-3 qualifying, over-fetch 10), friends profile (same + D8 fields on recentRounds), friends activity (SQL `total−course_par` diff → JS roundMath + `thru N` + total>0 guard), rounds GET (+`holes_played/par_played/to_par_through/is_partial/equiv_18`). 3 route tests (real requireAuth, db-singleton patch harness).
- **S2** client: Recent Rounds (Home), FriendProfile list, RoundHistory sheet, friends-activity card all render "47 thru 10 (+7)" for partials via server fields (legacy fallback preserved); HcpBadge trend plots the normalized equiv18−par18 series (tooltip shows raw "thru N"); Avg tile sub → "Per 18 holes"; Best tile labels 9-hole bests; handicap-milestone lib excludes partials + 9-hole rounds from "personal best"/"sub-80" claims (12/12 node tests). **Audit catch:** RoundScorecard header showed a partial as `total − coursePar` (−25 class) — fixed to par-played + "thru N".
- **S3** solo summary partial UX: "Partial round · N of 18 holes" header, amber banner w/ honest stats note, unscored holes amber-dashed + every grid cell taps back into scoring at that hole (min 44px), button → "Save partial round (N holes)"; zero-score guard from the -71 fix stays the floor.
- **S4** outing `/end`: records ≥9-PLAYED rounds (a 15-of-18 match round is now a stats citizen; no-shows/early-withdrawals still excluded), INSERT now carries `hole_pars`/`hole_handicaps` (rows self-sufficient for par-played), shots re-cleaned vs FINAL scores. **Fact-hygiene hardening:** `cleanPuttEntry` requires score>0 (phantom "0 putts" on unplayed holes closed), `cleanShotsForRound(shots, scores)` drops shot chains on unplayed holes (solo POST + /end pass scores; legacy callers unchanged). 6 new tests.
- **S5 gates:** 179/179 server vitest · 12/12 milestone node tests · client build+lint green · `node --check` on all 8 touched server files · handicap.js **zero diff lines** (git-verified) · **prod parity PROVEN**: all 49 tm_rounds are full 18s (SQL: 0 nine-hole, 0 odd-length, 0 zero-holes) → equiv18 ≡ total → no user's Avg/Best/3-RND AVG moves at deploy.
- **Honest residuals** (also §Status in the spec): outing partial recording has no e2e over that exact block (suite-green + code-reviewed; verify with a real ≥9-hole partial match on beta); client runtime = Matt's device after Vercel deploy; PATCH /:id/shots still cleans without scores (post-hoc editor, saved rounds only).

## [2026-07-17] feat | Marketing page BUILT — the-match.openscaffoldlabs.com (POST-LAUNCH-TODO #23)
- Matt: "build out the marketing page — find the url we decided on." Four-tool lookup found it in POST-LAUNCH-TODO #23: **`the-match.openscaffoldlabs.com`** (decided 2026-06-06, Dale) — NOT the app origin, NOT `thematch.app` (still TBD for referral short links).
- Built `marketing/` — a zero-dependency static single page (index.html, embedded CSS/JS, IntersectionObserver reveal honoring prefers-reduced-motion). Augusta-at-night language (night #070C09, gold gradient wordmark, Georgia-italic serif display). Sections: hero → feature pills → Eagle Eye (plays-like, club arcs) → Live Matches (formats, guests, never-lose-your-round, partial rounds) → The Caddie → Your Game Measured (WHS, SG, GamePlan, rivalries) → swipe gallery → closer CTA. CTA = "Play the beta" → the app origin (Matt: no App Store framing yet). No competitor names.
- `marketing/assets/` = the 11 App-Store screenshots from Matt's 07-17 session (`wiki/assets/app-store-2026-07/`), resized to 720px JPEG q82 (26–70 KB each; page total well under 1 MB).
- **Verified live in Chrome** (localhost:8899): hero, all four feature sections, gallery, closer, footer render correctly; reveal animations fire; real screenshots crisp.
- **DEPLOYED LIVE same session** — Matt pushed back on manual steps ("why do i have to do this?"), and rightly: `openscaffoldlabs.com` is Vercel-registered on Vercel nameservers, so DNS auto-provisions. Via Matt's authenticated CLI: `vercel link` → project **`the-match-marketing`** (OSL team, CLI-deployed, NOT git-integrated) → `vercel deploy --prod` → `vercel domains add the-match.openscaffoldlabs.com`. **Verified: HTTP 200 + correct `<title>` at https://the-match.openscaffoldlabs.com.** Redeploy = `cd marketing && vercel deploy --prod --yes` (documented in marketing/README.md). Remaining from #23: swap social bios / GolfNow materials to this URL.

## [2026-07-17 AM] feat | Marketing page REDESIGNED to the champagne-brief editorial language — promoted to root
- Matt shared his investment-brief .pages doc ("focus on the design... would love to see our marketing website with this same design"). Extracted the design system from the package preview/images: champagne radial cover w/ hairline frame + gold-gradient wordmark + M monogram + stat row, ivory interior pages, deep-forest serif display (web: Fraunces), tracked gold micro-labels ("SECTION THREE"), numbered features (01/08), outline pill badges, hairline rules. Brief CONTENT disregarded per Matt (no investor numbers on the site — stat rows use app facts only).
- Built at `/editorial` for side-by-side, then Matt: "get them in sync first before making it the main page." Content-parity diff found 3 gaps: the "Your Game, Measured" split (handicap+rivalry phones + WHS/SG/GamePlan/Rivalries bullets — now Section Four; numbered index renumbered to Section Five), sticky nav (persistent beta CTA), og:image. All added.
- Matt mid-build: cover should have the hero phone AND "instead of a static screenshot make it a short animation of the flyover view." Shipped a pure-CSS flyover: slow tee→green camera drift (scale 1.03→1.24 + translateY, origin on the flag, 11s alternate loop, reduced-motion → static; `.flyover` wrapper is video-swappable when a real screen capture exists). Verified animating via computed-transform sampling.
- **Two real bugs caught by pre-ship verification:** (1) page couldn't scroll — `body{overflow-x:hidden}` made body a wheel-swallowing scroll container (the decorative cover arc overflows horizontally); live CSS bisect in Chrome found it; fix = `overflow-x: clip` + `overflow:hidden` on the cover. (2) reveal-on-scroll could leave sections invisible after programmatic jumps — added load/scrollend sweep fallback.
- **SWAPPED TO ROOT + LIVE (verified 200s + content probes):** `/` = editorial design · `/dark/` = original Augusta-at-night page (kept) · `/editorial/` = redirect to root. Redeploy contract unchanged (CLI, not git-integrated).

## [2026-07-17 AM2] feat+fix | Marketing page finished out — real flyover video, Tour + Practice sections, brand polish, design-audit fixes
- **Brand polish (Matt):** nav wordmark → full polished-gold gradient (was two-tone w/ dark "The"); cover kicker → **"WHERE COMPETITION LIVES"**.
- **Real flyover video in the cover phone (Matt recorded it in the simulator):** found `Simulator Screen Recording - iPhone 17 Pro Max (listing) - 2026-07-17 02.44.43.mov` on the Desktop, re-encoded 51MB→**1.7MB** (584w H.264 crf26, muted, faststart) via imageio-ffmpeg's static binary (no brew ffmpeg on the Mac; avconvert's best was 20MB). `<video autoplay muted loop playsinline poster>` replaces the CSS Ken-Burns approximation; reduced-motion users keep the poster. **Verification caveat honestly logged:** the sandboxed Chrome can't decode ANY video (even chrome's native player spins), so playback was verified by proxy — ffmpeg decode-test clean, CDN serves byte-identical (sha1 match), 206 ranges OK, correct content-type; Matt confirmed visually ("looks great").
- **Section Seven · The Tour** (Matt: "add a section about live pga scores"): screenshotted his open simulator via `xcrun simctl io booted screenshot` (pixel-perfect), section = "The pros, live, one tab away." + gallery card. Tour png also filed to wiki/assets/app-store-2026-07/ for the listing set.
- **Section Five · Practice & Planning** (Matt: "useful before stepping foot on the course, knows ur play style, customized to each person"): "Your best round starts the night before." — Focus Areas + GamePlan phones; copy written from the actual screenshots (31% blow-up-hole diagnosis, "your 150-160 layup club"). Flywheel index → Six, Tour → Seven.
- **Design audit (Matt asked for the design-critique skill):** 6 findings, 6 fixed, deployed + verified: (1) 🔴 gold micro-label text measured 2.5–2.9:1 vs WCAG-AA 4.5 → new `--gold-text #7A6530` token (4.6–4.9:1 computed) for ALL text gold, decorative gold kept for rules/borders — the same gold/gold-text split the app's tokens.css makes; (2) dueling identical CTAs above the fold → nav CTA hidden until scrolled past the cover (IO-toggled fade); (3) anchor targets landed under the sticky nav → scroll-margin-top; (4) no keyboard focus indicators → gold :focus-visible rings; (5) page had no h1 → wordmark is the h1; (6) Tour phone one-off markup → shared .phone component. Stat sublabels/captions/footer ink-4→ink-3 (2.5→~5:1). **Residual: sub-880px mobile layout verified by code review only — sandbox Chrome won't resize; Matt's phone is the check.**
- POST-LAUNCH-TODO **#23 marked LIVE** (only the social-bio/GolfNow URL swap remains, Matt/Dale accounts). Dale brief PDF generated for handoff (outputs, not committed).

## [2026-07-17 PM] fix | App icon standardized to the M monogram across every surface
- Matt noticed Dale's watch-app work (feat/watch-w0) shipped a NEW icon (gold flagstick), when the M monogram has been the app's mark "for a long time." Audited: found THREE divergent icons — PWA/home-screen install = the gold M (`client/public/icon-512.png`, shipped 3mo ago as "Trophy Press v22"); Capacitor iOS shell = the Capacitor DEFAULT blue-X placeholder (never swapped since Phase-0 packaging); Dale's native+watch = the gold flag.
- Decision (Matt): the M everywhere. Canonical source = `icons-wip/22-final.png` — the 1024 master the PWA icon was exported from. Verified it IS the same artwork, not a variant: downscaled 22-final→512 vs the live PWA icon-512 = mean pixel diff 0.999/255, only 3.3% of pixels differ >10 (resample/compression noise). Matt viewed both side-by-side and confirmed. Chose the crisp 1024 master over upscaling the 512 PWA file (better App Store quality; same mark). 1024×1024, no alpha (App-Store-safe).
- Applied: (a) **main** — `client/ios/App/App/Assets.xcassets/AppIcon.appiconset/AppIcon-512@2x.png` (was blue-X) → committed+pushed `main`; (b) **feat/watch-w0** — `ios/TheMatch` + `ios/TheMatchWatch` AppIcon `icon-1024.png` (was gold flag) → committed+pushed the branch (`790e9b0`), commit message states plainly that the M was built over a month ago and has been the standing app icon ever since.
- Working tree clean, main==origin, no stash lost. Icon verified on main post-swap (1024, no alpha). PWA/web icons already the M — untouched.

## [2026-07-17 PM3] fix | Native sign-in dead on TestFlight — root-caused to a build-env dependency, hardcoded the native API origin
- Matt (TestFlight): was signed in, took the TestFlight update, app made him re-sign-in, and sign-in failed with "the string does not match the expected pattern." Locked out.
- **NOT Matt's code change and NOT this session's push.** Verified: (a) OTA `/api/v1/ota/updates` returns "no active bundle" → nothing OTA'd to the device; (b) no OTA-publish CI; (c) api.js (last changed ad51c45, 07-16) + Login.jsx (07-07) + capacitor.config.json are IDENTICAL between Matt's working build and Dale's build 3/4 (git-diff empty). The ONLY difference is the build-time env `VITE_API_ORIGIN` — committed nowhere. Matt's yesterday build was made with it exported (worked); Dale's rebuild (builds 3/4, watch consolidation/strip) was made without it → empty API origin → native calls resolve to `capacitor://localhost/api/…` → CapacitorHttp throws the pattern error → sign-in (and every backend call) dead.
- **Fix (`client/src/lib/api.js`):** native builds now fall back to `https://the-match-roan.vercel.app` (same origin the OTA config already uses) when `VITE_API_ORIGIN` is unset — self-contained isNativeBuild() detection (Capacitor global + UA + `__TM_NATIVE__`, no import cycle). VITE_API_ORIGIN still wins if set. NO-OP on web (isNativeBuild() false → API_ORIGIN ''). Build + lint green. Removes the fragile ambient-env dependency permanently.
- **CORRECTION to an earlier claim in this entry:** I first said "no OTA bundle" — wrong wording. A bundle DID exist (v1.0.1, published 07-17 05:25 UTC, built by Matt) but sat `active=false`, so nothing was being served. Matt caught it.
- **UNBLOCKED VIA OTA (no native rebuild needed).** Published v1.0.2 from main via `scripts/ota-publish.mjs` (dry-run first, then live). **Audit-verified, not tool-self-report:** (a) SQL — 1.0.2 `active=true`, 1.0.1 flipped `false`; (b) endpoint POST /api/v1/ota/updates with a builtin/native-1.0 device returns the 1.0.2 payload + checksum; (c) bundle URL HTTP 200, 6,922,549 bytes; (d) served checksum `28ff6fc0…` == sha256 of the built zip; (e) grep of the shipped JS confirms the backend origin is baked in. Devices pick it up on next app open. Rollback: `node scripts/ota-rollback.mjs --to 1.0.1`.
- **Honest nuance:** the OTA bundle works because `ota-publish.mjs` DEFAULTS `VITE_API_ORIGIN` to the prod backend at build time — NOT because of the api.js fix. The api.js fix (011a92f) is the durable guard for future native `.ipa` builds, where no such default exists. Both matter; don't conflate them.
- **CONFIRMED on device 2026-07-17:** Matt reopened the TestFlight app, it pulled v1.0.2, and **sign-in worked.** The OTA delivery path is now proven end-to-end (publish → endpoint → device → working auth) — the last unverified link is closed. Delivering a JS fix to a broken native build WITHOUT a rebuild is a validated capability now, not a theory.
- **Follow-through (f477dfd) — closed the structural hole rather than leaving it as a note.** Matt: "build at the highest level now so we don't have to come back and fix things later." The outage was ONE value with FOUR uncoordinated copies (api.js, capacitor.config.json x2, ota-publish.mjs) + no guard — the same shape as the -71 bug, which THIS repo's own partial-rounds spec warns about. Now: `client/src/lib/backend-origin.js` is the single source (canonical `BACKEND_ORIGIN` + PURE `resolveApiOrigin()`); api.js and ota-publish.mjs import it; capacitor.config.json (static JSON, can't import) is held by a drift guard that was **verified by deliberately breaking it** and watching the test fail. `shouldAutoResumeSolo()` extracted from Outing.jsx so the pull-to-refresh fix is pure + unit-tested instead of untested inline component logic. Gate: backend-origin 8/8, solo-round-phase 13/13 (4 new), client suite 0 failed, server 195/195, build + lint green. Behavior-neutral for the live bundle — no republish.
- **Still open (same 1.0.2 bundle, unverified on device):** the ORIGINAL pull-to-refresh bug — swipe-down on the Match hub should now just refresh instead of yanking into ActiveRound's summary. Matt to confirm.

## [2026-07-20] ops | Post-rotation recovery + OTA v1.0.3 (swing work + fixes to devices) — greenlit by Matt
- Context: Dale/Kimi's 07-19/20 session (their report, filed by Matt) rotated the Supabase DB password after an accidental exposure. Our local `.env` died silently — **verified** (`password authentication failed`) — which meant `ota-publish.mjs`/`ota-rollback.mjs` were both dead: the live 1.0.2 bundle had NO rollback path. Matt greenlit recovery + "build at the highest level."
- **.env restored safely:** backup first → `vercel env pull` production to a TEMP file (correct project verified: prj_xNVh…/the-match) → key-by-key merge, pulled values win for shared keys, local-only keys (SUPABASE_URL, SERVICE_ROLE_KEY) preserved, key NAMES only ever printed. 7 keys updated. Verified: DB connect OK against the rotated password (reads tm_ota_bundles — the exact query rollback needs) + storage service key OK (bucket listable). Caveat noted: CLIENT_ORIGIN now = prod value; old value in the backup if local-dev CORS complains.
- **Native-permission trap checked BEFORE publishing swing JS to old binaries:** SwingCapture.jsx uses getUserMedia+MediaRecorder; a binary without usage strings would be KILLED by iOS on camera access. Verified NSCameraUsageDescription + NSMicrophoneUsageDescription shipped in ad51c45 (07-16 12:54) — a day before ANY TestFlight build was compiled → present in Matt's binary. Safe.
- **Full gate on main @ 3cc2820 before publish:** fresh npm install (new swing deps), client suites all green (62+12+54+36, 0 failed), server 195/195, build + lint green.
- **OTA v1.0.3 published + audit-verified independently of the script's self-report:** SQL shows 1.0.3 active (1.0.2, 1.0.1 inactive); endpoint offers 1.0.3 to a 1.0.2 device; served zip sha256 == local build (single unique checksum); 9.25MB (up from 6.9 — MoveNet ONNX now bundled). Ships: Swing Intelligence V0–V3 + pose metrics, sign-in origin fix, pull-to-refresh fix, partial rounds, M icon web assets. Rollback verified AVAILABLE: `node scripts/ota-rollback.mjs --to 1.0.2`.
- **Hygiene catch from own checklist:** `.env.backup-2026-07-20` (contains the OLD rotated credentials) was sitting untracked-and-committable — .gitignore now covers `.env.backup*`. Dale-side items still open from their report: revoke the exposed GitHub classic token (their account).

## [2026-07-20] schema | NotebookLM 50-cap prune — 50→33 sources (Matt-approved plan, executed + verified)
- Inventory of all 50 presented to Matt; greenlit: 19 shipped/closed pages → 2 rollups (established f5/handoffs pattern). `shipped-specs-rollup.md` (15 pages: June handicap wave ×5, plays-like ×2, club arcs, rings, putt capture, EE tokenization ×2, superseded EE plans ×2, SG map-tap) + `closed-audits-rollup.md` (audits 04-29/05-07, closed audit-fixes proposal, shipped match-page plan). Pages remain in wiki/git untouched — only notebook SLOTS consolidated.
- Executed with per-step verification: 19 deletes each confirmed by returned source-id; post-delete count exactly 31; both rollups added + processed; **final count 33** (17 slots free). Content probe (anti-pattern #12): asked for the plays-like wind coefficients — answer cited the asymmetric 1%/0.5% correction + 250yd carry cap, which exist only inside the rollup → rolled content is queryable.
- Manifest: 19 full-filename exclude_paths (no prefixes — can't catch the rollups; the 2026-07-10 f5 prefix trap is the precedent). index.md: 🗜 legend + markers on all 19 + rollup entries. Contract: edit a 🗜 page → regenerate its rollup.

## [2026-08-07] fix | On-course GPS DEAD on TestFlight — native plugin shipped by OTA to a binary that doesn't have it (`dafae93`, OTA v1.0.4)
- Matt: "last time i used the match on a course the gps was not working at all." Diagnosed from the **actually-served artifact**, not the source tree: downloaded active OTA bundle 1.0.3 and read its JS. `useActiveMatchGuard-*.js` (the chunk carrying `lib/geolocation.js`) does `import("./index-DlklrRRs.js").then(e=>e.Geolocation)`; that chunk is `registerPlugin("Geolocation", …)`. So on native, EVERY GPS call goes through the **native** `@capacitor/geolocation` plugin.
- **Root cause:** that plugin is a NATIVE dependency added 2026-07-16 (`9c6fe4b`). `tm_ota_bundles` shows 1.0.1 / 1.0.2 / 1.0.3 ALL published with `min_native_version = 1.0.0` — a declaration that the JS is safe on the oldest binary, which was compiled ~6h BEFORE the plugin existed. `docs/OTA-RUNBOOK.md` states the exact rule that was missed: *"What OTA can NOT ship: native changes (new plugins…)"* + `--min-native` is the gate. Corroborating: the synced `client/ios/App/App/capacitor.config.json` (gitignored, written by `cap sync`, mtime Jul 17 00:46 — BEFORE `Package.swift`'s Jul 17 18:53) carries `packageClassList: [AppPlugin, HapticsPlugin, KeyboardPlugin, SplashScreenPlugin, StatusBarPlugin]` — no GeolocationPlugin, no PushNotificationsPlugin, no CapacitorUpdaterPlugin. `cap sync` never re-ran after the plugin landed.
- **Why it presented as "no GPS" rather than an error:** Capacitor rejects an unregistered plugin with `"Geolocation" plugin is not implemented on ios`. That rejection has no `.code`, so EagleEye's `err.code===1 / ===2 / else` handler fell through to `setGpsError('timeout')` — permanent "acquiring", all round, no diagnostic.
- **Fix (`dafae93`, client/src/lib/geolocation.js only):** two guards, both fail-OPEN to `navigator.geolocation` (the path that worked on course pre-native-shell). (1) gate the native path on `Capacitor.isPluginAvailable('Geolocation')` — what the BINARY actually has — not `isNativePlatform()`; (2) if the plugin path rejects anyway, latch native off for the session and re-run through the WebView, re-pointing the SAME watch handle so a caller's stored `clearWatch` reference stays valid. No-op on web/PWA and on any binary that does carry the plugin.
- Gates: client lint 0, build green, client suites (54/36/…) 0 failed, server 195/195.
- **OTA v1.0.4 published + audit-verified independently of the script's self-report** (anti-pattern #12): (a) SQL — 1.0.4 `active=true`, 1.0.3 flipped `false`; (b) endpoint offers 1.0.4 to a 1.0.3 device; (c) served zip sha256 `b7a9bab5…` == published checksum; (d) grep of the SERVED bundle confirms `isPluginAvailable("Geolocation")` + the fallback warn string are in it. Rollback: `node scripts/ota-rollback.mjs --to 1.0.3`.
- **HONEST RESIDUAL — not verified:** whether Matt's binary truly lacks GeolocationPlugin. Counter-evidence exists: `CapacitorUpdaterPlugin` is ALSO absent from that `packageClassList`, yet OTA demonstrably works on his device — so either the archive was built from a fuller sync than this Mac holds, or Capacitor 8 SPM registers by another path. The fix is correct either way (it's a no-op when the plugin IS present), but the CAUSE is confirmed only by Matt's next round. If GPS still fails on 1.0.4, the cause is elsewhere and the WebView path is the thing to instrument.
- **Open follow-ups (NOT done):** (1) **push notifications are likely dead on the current binary for the identical reason** — same missing packageClassList entry, never validated on device; (2) the real repair is `npx cap sync ios` → verify packageClassList carries GeolocationPlugin + PushNotificationsPlugin + CapacitorUpdaterPlugin → bump binary → new TestFlight build; after that, any OTA bundle needing a native plugin gets a correct `--min-native`; (3) **`min_native_version` has been `1.0.0` on every bundle ever published** — that field is currently decorative. It needs to be a deliberate per-publish decision, and `ota-publish.mjs` should refuse a bare default when the diff touches a `@capacitor/*` or `@capgo/*` dependency.

## [2026-08-07 PM] fix | The dead "Enable Location" button — plugin hangs on `.notDetermined`; native path now gated + deadlined (`4e240e8`, OTA v1.0.5)
- Matt, after 1.0.4: *"the green box that says enable location for the nearest course default used to be clickable which turned gps on, right now it is not clickable and the app is not checking for gps signal."* Wiring was fine (`PlayStart` button → `onRequestLocation` → EagleEye `requestLocation`); the button was firing and the PROMISE NEVER SETTLED.
- **Root cause, in the plugin's own iOS source** (`node_modules/@capacitor/geolocation/ios/.../GeolocationPlugin.swift:223`): `handleLocationRequest` calls `addLocationCallback` — which does `capacitorBridge?.saveCall(call)`, parking the JS promise — and THEN switches on `authorisationStatus` with `default: break` covering **`.notDetermined`** (and `nil`). On a first run the call is saved and never resolved, never rejected. No fix, no error, no iOS dialog, and therefore no `gpsError` → the banner at EagleEye:2553 never renders → the button looks inert. **This is a distinct bug from the 1.0.4 one** (missing plugin); 1.0.4's `isPluginAvailable` gate does nothing here, because the plugin IS present.
- **Fix (`4e240e8`, same one file):** (1) take the native path ONLY when `checkPermissions()` already reports `granted` — `prompt`/`denied`/unknown defer to `navigator.geolocation`, which reliably raises the iOS dialog and is how permission got granted for months pre-shim; (2) race EVERY native call against a deadline (3s probes, caller `timeout`+5s acquisition) — a stall latches native off for the session and redoes the request on the WebView path; (3) a native watch that registers but delivers no first fix inside the deadline also falls back, re-pointing the SAME handle so a caller's stored `clearWatch` stays valid. Guard (2) is deliberately broader than the one bug we can name — any plugin wedge now degrades instead of hanging.
- Gates: client lint 0, build green, client suites 0 failed. **OTA v1.0.5 audit-verified** (anti-pattern #12): SQL 1.0.5 `active=true` / 1.0.4 flipped false; endpoint offers 1.0.5 to a 1.0.4 device; served zip sha256 `791fd05d…` == published checksum; grep of the SERVED bundle confirms all four new guard strings. Rollback: `--to 1.0.4`.
- **Method note worth keeping:** 1.0.4 was diagnosed from the served OTA artifact and was *correct but incomplete* — it fixed the failure mode I could prove and I reported it as the fix. The second mode was only found by reading the PLUGIN'S OWN SWIFT SOURCE rather than reasoning about its documented contract. When a promise-based native bridge "does nothing," read the native switch statement for the branch with no arm.
- **Still unverified (Matt's device is the only oracle):** which of the two modes his binary actually had. Both are now defended. If GPS is still dead on 1.0.5, native is exonerated and the WebView path itself is the suspect — instrument there next.

## [2026-08-08] fix | ROOT CAUSE: course vendor breaking API change (string ids + coordinates removed) — the blank hole map, finally (`1ece43f`, `2a22108`, `ebbaa7f`, migration 051, OTA 1.0.11)
- **The finding, from our OWN tm_course_search_cache history.** Between 2026-07-17 and 2026-07-23 the course data vendor made TWO breaking changes with no notice. Verified against their live API and our cache: (1) course ids went INTEGER → ALPHANUMERIC STRING (`'la tourette'` → `23978` on 07-17, → `"pjvj6c9d"` on 08-08); (2) `location.latitude`/`longitude` were REMOVED and a street `address` added (07-17 keys `…,id,latitude,longitude,state` → 08-08 keys `…,id,state`).
- **Why it blanked the hole map SILENTLY.** EagleEye anchors its OSM/Overpass load on the course coordinates, falling back to geocoding `club_name, city, state`. For `"La Tourette Golf Club, Staten Island, NY"` Nominatim returns **ZERO** results (measured). So `gc === null` → EagleEye returns at "no location anywhere" (EagleEye.jsx:1317) → `courseGeocoded` stays null → HoleMapGL's init effect bails on `!geocoded`. The map never initialises: no tiles, no error, no retry card, just the `--tm-ee-map-bg` background — which is dark green. **A dead map and a loading map are pixel-identical.**
- **Fix, server-side; the client needed no change** (it already prefers course lat/lon over its own geocode). `resolveCourseCoords()`: vendor coords → `tm_course_geocode` cache → geocode `location.address` (the only form that resolves — `1001 Richmond Hill Rd…` → 40.5731,-74.1467, ~340 m from the vendor's own pre-removal value). Never throws; a course still loads without a map. Coordinate-less payloads are NOT cached (would pin the bug for the 30-day TTL). `Number(req.params.id)` → `String` — every string-id course was silently skipping the cache on BOTH read and write, hitting the rate-limited vendor on every load.
- **Migration 051 (applied to prod + verified):** `tm_course_cache.course_id` bigint→text (old numeric ids survive as digit strings); new `tm_course_geocode` (text pk, lat/lon, source vendor|nominatim|manual, query, CHECK constraints; `manual` is never overwritten). Seeded La Tourette from the vendor's OWN 07-17 coordinates so the morning round does not depend on Nominatim being up.
- **Search: article-prefixed courses were unfindable typed as one word.** Vendor matches token prefixes ANDed: `'la t'`→4 hits incl. La Tourette, `'latourette'`→0. Zero-result space-free queries (len≥5) now retry with one space after char 2 then 3 (covers La/Le/De/Du/El/Mc/Mac/Van), filtered by punctuation-normalised containment of the original so it can't drag in "La Tuque". Also deleted 17 cached zero-result rows that would have served "no matches" for a week.
- **Also this session (separate causes, all real):** USDA NAIP imagery went down — `gis.apfo.usda.gov` accepts TCP 443 then drops the TLS handshake, confirmed from two independent networks each with a positive control. Added primary→fallback imagery (USGS National Map; public domain, no key — NOT ESRI, whose keyless endpoint is the commercial ToU violation removed in `57e1ba1`). USGS caches only to z16 so the fallback is coarser; a BACKUP IMAGERY badge says so, and NAIP stays primary so full res returns by itself. `lib/import-deadline.js`: a dynamic `import()` in Capacitor can HANG rather than reject (WKURLSchemeHandler need not fail an unsatisfiable request) — that killed `@capacitor/geolocation` (fixed by static import) and `maplibre-gl`'s retry loop (a hang never throws, so the catch never ran and the Leaflet fallback never engaged).
- **VERIFIED ON PROD, not self-reported:** `/api/courses/search?q=latourette` → 1 result, La Tourette Golf Club, Staten Island NY, `pjvj6c9d`. `/api/courses/pjvj6c9d` → lat 40.5761 lon -74.14681, 4 male tees, Blue 6708 yds, 18 holes. Migration: `course_id` type=text, geocode row present. Overpass for that bbox → 18 holes (refs 1–18), 20 greens, 33 tees. OTA 1.0.11 served to a 1.0.10 device, served sha256 == DB checksum, all three guards grep-confirmed in the SERVED bundle. Server suite 195/195; client lint 0, build green, suites 0 failed.
- **Process failure worth keeping.** Matt said three times that the map worked last week. I kept auditing my own diffs and the imagery provider instead of asking what CHANGED — the answer was in his own database and one query on the search cache would have found it hours earlier. Prior sessions' anti-pattern "verify before claiming" needs a sibling: **when the user says it worked before, diff the WORLD, not just your work.** Also: I shipped 1.0.4 and 1.0.5 as "fixes" when they were untestable hypotheses, and only built the on-screen diagnostic (which found the real bug in one round trip) on the fourth attempt. The instrumentation should have been bundle one.

## [2026-08-08 PM] fix | The delivery model, not the bundle: same-launch OTA apply + OTA stats that aren't blank (`e897bee`, OTA 1.0.12)
- **Why the morning round still failed even though the fix was live.** 1.0.10 published 05:14:59; device downloaded it 05:15–05:16. **1.0.11 — the fix — published 05:31:10, AFTER that check-in.** Device downloaded 1.0.11 at 11:14 (7:14 AM ET) and, per Capgo's model (download on one launch, swap on the NEXT), kept running 1.0.10 for the entire round. No bundle was wrong. The DELIVERY was. "Force-quit and reopen twice" on the first tee is not a fix, it is a wish.
- **Same-launch apply (`applyPendingBundleIfSafe`, client/src/lib/native.js).** After `notifyAppReady`, if `getNextBundle()` reports a queued bundle whose version differs from `current()`, call `reload()` — the plugin's own documented mechanism for applying a queued bundle immediately. The clean alternative, the `directUpdate` config flag, lives in the NATIVE `capacitor.config.json` and therefore cannot ship over the air; this can, which is the entire point. **Bootloop safety** (a self-reloading app is far worse than a stale one), all required: queued bundle differs from running; no round in progress (never yank the webview out from under scoring); same version not attempted within 30 min (the real guard — a bundle that fails to boot is rolled back and would otherwise re-queue forever). Any check failing = do nothing, so the worst case is exactly today's behaviour.
- ⚠️ **Honest limit: this does NOT retroactively fix the 1.0.11 → 1.0.12 hop.** 1.0.11 has no self-apply code, so 1.0.12 still lands one launch late. Same-launch behaviour begins with the update AFTER 1.0.12.
- **OTA stats were blank since July — and that blindness had a cost.** The plugin BATCHES and POSTs a JSON **array** (`CapgoUpdater.swift flushStatsQueue`: `parameters: eventsToSend`). The route read `req.body.device_id` on that array → `undefined` → every column an empty string, one row per BATCH rather than per event. Consequence: when a device sat on a stale bundle through a whole round there was no way to ask "what is that phone running" — it had to be reconstructed from bundle publish timestamps vs check-in timestamps. Now handles array or single object, one row per event. **111 legacy blank rows remain** (left in place; they are honest history).
- **Settings footer now shows the running bundle version** — "which build is this phone on" is answerable from the phone.
- **Verified, not self-reported:** batched-array POST → 2 correctly-populated rows; single object → 1 (back-compat); test rows deleted. Prod stats endpoint after deploy → populated row (older rows in the same window are blank = pre-deploy device check-ins, which dates the cutover). OTA 1.0.12 offered to a 1.0.11 device; served sha256 == published checksum; all four new guard strings grep-confirmed in the SERVED bundle. server 195/195; client lint 0, build green, suites 0 failed (62/12/54/36).
- **Also confirmed today by driving the app in a real browser** (the verification skipped all night): planting a coordinate-less La Tourette → reload → `tm-shared-course` cleared, guard fires; course fetch returns lat 40.5761/lon −74.14681; Eagle Eye header renders the club name; BACKUP IMAGERY badge + USGS attribution show; **8 satellite tiles fetched, decoded and cached** in `naip-tiles-v1`; OSM geometry cached for `pjvj6c9d`. Caveat recorded: the map still LOOKS green in a captured screenshot and a canvas pixel read returns all-black — the WebGL capture artifact already logged 2026-07-10. Tile-cache entries, not screenshots, are the reliable signal in a browser; iOS screenshots do capture WebGL, which is why Matt's were real evidence and mine were not.
